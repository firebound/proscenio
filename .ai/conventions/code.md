...
## Blender panel UI (N-panel)

How Proscenio panels read in the sidebar, including the narrow-width behaviour Blender's API forces.

- **"Picker" vs "Target".** The field where you choose a cross-panel selection is the *picker*, and only its owner panel names it that (the Skeleton panel owns the Active Armature picker). Every other panel that *acts on* that selection calls it the **target** and declares it with a one-line readout "Target: <source-panel> <name>" (e.g. "Target: Skeleton doll.armature"), via the shared `draw_target_readout` helper. The readout names both what the panel targets and where it is set. The owner panel never shows a "Target:" line - it holds the picker widget itself.
- **UIList row labels left-align via a split.** A row drawn as a single `row.operator(..., emboss=False)` stretches and *centers* its text (which hides any depth-indent prefix). Wrap the label operator in a `LEFT`-aligned sub-row inside a `layout.split(...)` so the name hugs the left edge; put the trailing affordance (favorite star, connectivity flags) in the split's right sub-row. See the Outliner and Skeleton bone lists.
- **Header extra icons drop on a narrow panel.** The status badge + `?` help drawn in `draw_header_preset` (via `draw_subpanel_header`) overlap the title when the N-panel is narrow. Gate them on the panel-callback `context.region.width` (pass `context` into the helper - never read global `bpy.context.region`, which can resolve to a different editor's region in a multi-editor layout): skip the icons below a width threshold so the title keeps the header. This mirrors how a narrow Blender header sheds its right-side affordances.
- **A dynamic header title cannot truncate; width-gate the suffix instead.** Only a native `bl_label` truncates (clips characters) when narrow; a label drawn in `draw_header` *vanishes entirely* once cramped, and Blender renders `draw_header` content to the LEFT of `bl_label` (so a static-`bl_label` + dynamic-`draw_header` split reads reversed). To show a dynamic name in a panel header (e.g. "Skeleton: <name>"): set `bl_label = ""`, draw the whole title in `draw_header`, and width-gate the dynamic part - below a threshold drop the suffix and render just the short base title (which survives narrow widths), restoring the full title when widened. A true per-character truncation of a dynamic header title is not achievable in Blender's panel API.

### Interactive-tool gesture cheatsheet

Every modal "interactive tool" (currently **Quick Armature**, **Automesh Authoring** / interactive mesh, **Manual Mesh** / draw-with-vertices, and **Edit Weights**) shows a live gesture cheatsheet built from the shared `chord` primitive (`operators/_status_bar.py`), rendered both on Blender's STATUSBAR (the canonical modal-hint home) and mirrored on the tool's N-panel while the modal runs.

- **`chord` spacing - two non-obvious rules.** A keycap icon (`EVENT_CTRL`, `EVENT_X`, `EVENT_ESC` ...) butts straight up against its label text, so each non-empty text needs a **leading space** (`f" {text}"`) to read "Ctrl grid snap" rather than the run-together "Ctrl"+"grid snap" - labels keep leading spaces (the same trick indents the bone list). And the row **must set `row.alignment = "LEFT"`**: the default `EXPAND` splits the row width equally between the labels, pushing a combo's meaning ("Shift + ... unparented") to the panel's right edge with a huge gap (neither `align=True` nor `align=False` changes that - only `alignment` does). Author each gesture as one attached `(icon, text)` part; carry a `+` / `/` joiner as the modifier's text.
- **Panel cheatsheet mirror.** To echo the status bar on the tool's N-panel, draw it in a collapsible `header, body = layout.panel("<id>", default_closed=True)` section (closed by default so it stays out of the way) and emit the chords into `body` only when the modal is live - gate on the operator's `_statusbar_appended` class flag (set by `append_statusbar_draw`). Canonical example: `_draw_quick_armature_shortcuts` in `panels/skeleton.py`. All four interactive tools carry both the status-bar cheatsheet and the panel mirror.
