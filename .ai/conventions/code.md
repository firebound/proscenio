...
Every modal "interactive tool" (currently **Quick Armature**, **Automesh Authoring** / interactive mesh, and **Edit Weights**) shows a live gesture cheatsheet built from the shared `chord` primitive (`operators/_status_bar.py`), rendered both on Blender's STATUSBAR (the canonical modal-hint home) and, optionally, mirrored on the tool's N-panel while the modal runs.
