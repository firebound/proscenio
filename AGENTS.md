...
3. **No GDExtension. No native runtime.** The Godot plugin runs only at editor import time. Generated scenes use built-in nodes only (`Skeleton2D`, `Bone2D`, `Polygon2D`, `Sprite2D`, `Node2D`, `AnimationPlayer`, `AnimationLibrary`). Operational test: open a generated `.scn` in a stock Godot project without the plugin installed - it must work.
