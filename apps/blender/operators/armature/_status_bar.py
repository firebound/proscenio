...
def emit_chord_layout(
    ...
    One single cheatsheet (no sub-modes): the draw gestures, the right-click
    chain pick, then the exit gestures. Uses Blender's native ``EVENT_*`` /
    ``MOUSE_*`` icons via the shared ``chord`` primitive so the hint visually
    matches Blender's own modal status bar (knife tool, loop cut, etc).
    """
    _emit_draw_rows(layout, cls)
    # Right-click selects a bone (native); the next draw chains from it.
    chord(layout, ("MOUSE_RMB", "select bone = chain from it"))
...
def _emit_draw_rows(
    ...
    Each gesture attaches its meaning to the key/mouse icon in a single label
    (``(icon, text)``) - the automesh status bar's style - so the icon and its
    text keep Blender's native spacing instead of gluing (the split
    ``(icon, "")`` + ``("", text)`` form packed them together). A modifier in a
    combo carries the ``+`` / ``/`` joiner as its text.
    """
