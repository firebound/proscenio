...
// A section's index.md (docs/<NN-section>/index-<lang>.md) is the landing page
// its wrapping sidebar category links to (sidebars.ts). Docusaurus absorbs a
// folder index as a *generated* subcategory's link automatically, but the root
// index of an autogenerated dirName leaks through as a stray leaf that
// duplicates the explicit category label. List those root-index doc ids here so
// sidebarItemsGenerator can drop the stray while `link` keeps the landing page.
// tools/index is not listed: it is linked from toolsSidebar but never sits
// inside an autogenerated dirName, so it never leaks a stray leaf to strip.
const SECTION_LANDING_IDS = new Set(['guides/index', 'project/index', 'content/README']);
    ...
    [
      'classic',
      {
        docs: {
          ...
          // Strip a section's root index (SECTION_LANDING_IDS) from every
          // autogenerated list; the wrapping category's `link` already surfaces
          // it as the landing page, so leaving it in duplicates the label.
          sidebarItemsGenerator: async ({defaultSidebarItemsGenerator, ...args}) => {
            const items = await defaultSidebarItemsGenerator(args);
            return items.filter(
              (item) => !(item.type === 'doc' && SECTION_LANDING_IDS.has(item.id)),
            );
          },
