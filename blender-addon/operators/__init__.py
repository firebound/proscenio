...
class PROSCENIO_OT_quick_armature(bpy.types.Operator):
    ...
    Bypasses Edit Mode + Shift+E for sketch-iteration on new rigs. On
    first invoke creates a 'Proscenio.QuickRig' armature; subsequent
    drags extend it. Hold Shift on press to auto-parent the new bone
    to the previous one in the chain.

    Modal flow: LEFTMOUSE press records the head world point on the
    z=0 plane, LEFTMOUSE release records the tail and creates the
    bone. ESC / RIGHTMOUSE exits.
    """
    ...
    def invoke(self, context: bpy.types.Context, _event: bpy.types.Event) -> set[str]:
        if context.area is None or context.area.type != "VIEW_3D":
            self.report({"WARNING"}, "Proscenio: must run in a 3D viewport")
            return {"CANCELLED"}
        type(self)._drag_head = None
        type(self)._last_bone_name = ""
        type(self)._shift_held = False
        if self._ensure_armature(context) is None:
            self.report({"ERROR"}, "Proscenio: failed to create QuickRig armature")
            return {"CANCELLED"}
    ...
    def _handle_leftmouse(
        ...
        if event.value == "PRESS":
            type(self)._drag_head = _mouse_event_to_z0_point(context, event)
        ...
        if head is None:
            return {"RUNNING_MODAL"}
        tail = _mouse_event_to_z0_point(context, event)
        if tail is None:
            return {"RUNNING_MODAL"}
        from mathutils import Vector

        if (Vector(tail) - Vector(head)).length < 1e-4:
            self.report({"INFO"}, "Proscenio: bone too short, skipped")
            return {"RUNNING_MODAL"}
    ...
    def execute(self, _context: bpy.types.Context) -> set[str]:
        ...
        # The redo panel calls execute on undo/redo; nothing to replay
        # because each bone was its own undoable edit-mode toggle.
        return {"FINISHED"}
    ...
    def _create_bone(
        ...
        *,
        ...
        # Drop other selections so the mode toggle targets the rig only.
        ...
        try:
            ...
            if parent_to_last and last and last in edit_bones:
                ...
                # use_connect=False -- head stays where the user clicked
                # rather than snapping onto the parent's tail.
        ...
        if bone_name:
            ...
            self.report({"INFO"}, f"Proscenio: '{bone_name}' added to {_QUICK_RIG_NAME}")
