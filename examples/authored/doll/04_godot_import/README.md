...
| File | Origin | Notes |
| --- | --- | --- |
| `Doll.gd` | hand-authored | the reimport-merge work documentation-by-example wrapper; auto-plays `idle` on ready |
| `Doll.tscn` | hand-authored | wrapper scene that instances the imported `.proscenio` scene |

Neither file is regenerated by any script. Both survive Proscenio reimports by living **outside** the imported scene (the reimport-merge work Option A). Reimport clobbers the inner generated `.scn`; the wrapper is yours.
