...
> Migration of the flat-layout fixtures into this subdirectory is intentionally deferred. The blast radius spans `packages/fixtures/`, every surviving spec's TODO, and the per-fixture wrapper paths in Godot - not worth one big refactor commit. New fixtures land here directly; legacy fixtures move when the next reason to touch them coincides with the rename.

## Pattern (reference: existing flat fixtures)

```text
<fixture_name>/
+- <fixture_name>.blend                   [SOURCE - built by packages/fixtures/<name>/]
...
See [`packages/fixtures/README.md`](../../../packages/fixtures/README.md) for the build-script conventions.
