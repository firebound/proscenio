...
- [x] `scripts/fixtures/doll/export_psd_manifest.py` - bpy: opens `doll.blend`, walks every mesh, projects world XZ bbox to a top-left PSD canvas at `PIXELS_PER_UNIT=100`, emits `examples/authored/doll/01_to_photoshop/doll.photoshop_manifest.json` matching schema v1 (kind=polygon for every mesh; sprite_frame deferred until the doll grows hframed eyes). Layer paths point at `render_layers/<name>.png`.
- [x] `examples/authored/doll/01_to_photoshop/doll.photoshop_manifest.json` - generated, schema-valid, references `render_layers/<name>.png`.
...
- [x] Manual smoke test in Photoshop: 22 layers placed correctly, JSX exporter round-trips identical positions / sizes (within 2 px from Workbench AA edge bleed). Round-trip output lands at `examples/authored/doll/02_from_photoshop/export/` (gitignored) when `proscenio_export.jsx` runs on the imported PSD.
