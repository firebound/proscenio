...
- [ ] `properties/__init__.py` (404 LOC) — split the three PropertyGroup classes into their own files. `properties/object_props.py` (sprite + slot + driver fields), `properties/scene_props.py` (atlas + outliner + validation results), `properties/validation_issue.py` (the 20-line `ProscenioValidationIssue`). The `__init__.py` keeps the `_classes` tuple, register/unregister, and the PointerProperty wiring; everything else moves out.
- [ ] `core/bpy_helpers/atlas_io.py` (308 LOC) — split into `atlas_collect.py` (`SourceImage`, `collect_source_images`, `_collect_mesh_uvs`, `_find_first_image`), `atlas_compose.py` (`compose_atlas`, `write_manifest`), `atlas_manifest.py` (`Placement`, `read_manifest`). `bpy_helpers/__init__.py` re-exports nothing — callers update their imports.
- [ ] `core/bpy_helpers/sprite_frame_shader.py` (337 LOC) — extract the bpy-free cell math (`cell_size`, `cell_offset_x`, `cell_offset_y`) into `core/sprite_frame_math.py` (top-level, bpy-free, gets its own pytest unit test). The shader graph IO (`ensure_slicer_group`, `apply_slicer_to_material`, `remove_slicer_from_material` and the `_*` node-tree helpers) stays in `bpy_helpers/sprite_frame_shader.py`.
- [ ] `panels/active_sprite.py` (293 LOC) — split the per-mode draw helpers into `panels/_draw_sprite_frame.py`, `panels/_draw_polygon.py`, `panels/_draw_preview_shader.py`. Each exposes a single `draw_body(layout, context, obj)`. The `PROSCENIO_PT_active_sprite` class shrinks to dispatch only.
- [ ] Pytest 139 stays green; mypy strict passes; ruff clean. No behavior change, no public-symbol rename. Test files import from the new module paths only when a test is actually exercising the moved code (no shim re-exports).
