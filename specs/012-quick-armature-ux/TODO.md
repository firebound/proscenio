# Quick Armature UX - TODO

Quick Armature UX overhaul. See [STUDY.md](STUDY.md) for the full design + decisions D1-D16 (D1-D9 locked at planning time, D10-D15 added with the second wave, D16 emerged mid-iteration). The first cut + second wave + 9 iterative refinement commits shipped together on PR #50. The first cut closed the "inviabiliza" gap surfaced in [tests/MANUAL_TESTING.md:167-177](../../tests/MANUAL_TESTING.md#L167-L177); the second wave layered the productivity polish; the refinement log below records each post-second-wave delta with commit hash + user-feedback rationale.
...
- [x] D9 - shipping = first cut (preview + lifecycle hygiene + Front-Ortho + cheatsheet) then second wave (modifiers + naming + in-modal undo + axis lock + grid snap + panel). Each tier shipped as a separate PR.
...
## the first cut - visual feedback + lifecycle hygiene (Blender)
...
- [x] `MOUSEMOVE` updates `_cursor_world` (with grid-snap + axis-lock post-processing in the second wave) + `area.tag_redraw()`. No timer.
...
- [x] Backlog "Quick Armature: Front-Ortho UX guard" entry marked absorbed by this spec.

## the second wave - modifiers + naming + in-modal undo (Blender)

Branch: `feat/spec-012.1-quick-armature-feedback` (kept same branch; PR #50 carries the first cut + second wave + refinements). **SHIPPED** across commits `cefd30d`, `265b59b`, `16c7995`.
...
- [x] [`.ai/skills/blender-dev.md`](../../.ai/skills/blender-dev.md) gained both the modal-overlay pattern (the first cut) and the modal-hint placement convention (refinement).

**Codebase audit (drive-by from the first cut PEP 563 bug) - CONCLUDED**:
...
## Refinement log (post-second-wave iterative feedback)

PR #50 carried the first cut + second wave on the same branch, then absorbed 8 refinement commits driven by manual-smoke feedback rounds. Each is a deliberate delta vs the STUDY-locked design, so they are listed here so a future reader can see what the operator actually does today vs what the original D1-D15 decisions said.
...
| Commit | Change |
| --- | --- |
| `c05abba` | this spec STUDY + TODO planning artifacts. |
| `03c3606` | `.ai/skills/blender-dev.md` Modal overlay pattern subsection. |
| `25a5590` | First MANUAL_TESTING smoke log (post-first-cut). |
| `3189f61` | PEP 563 root cause refined in BUGS_FOUND.md. |
| `cefd30d` | PG + panel sub-box (D15). |
| `265b59b` | the second wave operator changes (D10 invert + D11 axis lock + D12 grid snap + D7 undo + D2 prefix). |
| `f8813f2` | the first cut operator rewrite (preview + lifecycle + Front-Ortho snap + cheatsheet). |
| `88b9b8d` | Conventions PEP 563 rule + MANUAL_TESTING the second wave status block. |
...
- Auto-attach underlying mesh / sprite (couples to the slot system).
