...
- **Revise the in-panel tooltip and `?` help copy** so each panel's button explains the panel and subpanels explain their own specifics without leaking, and fix the stale status-badge legend that still names `TOOL_SETTINGS`.
- **Clarify the Reproject UV / UV bounds / Texture Region purpose** in help.
- **Fix the three in-code help strings that contradict the code** (driver-axis labels say "local rotation" though the operator forces world space; the `show_provenance_overlay` description says the GPU overlay "ships later" though it shipped; the bind error string says "Skinning panel > Bind mode" though the panel is now Weight Paint).
- **Guard Select Issue Object** against an out-of-view-layer object.
- **Fix the Help / Diagnostics wrong help topic** (both hard-code `pipeline_overview`, which has no table entry), route Run Smoke Test through the report gate, and drop the Diagnostics placeholder copy.
- **Hide the Texture Region subpanel** for mesh elements.

_The 2026-06-16 QA-walk reconciliation flagged the tooltip-copy revision, the stale status-badge legend, and the Texture Region hide-for-mesh to fold in when this spec was next opened - they are now in the Scope list and the assessment above and below. The shared-Automesh-parameter surfacing it also flagged stays in [`backlog.md`](../backlog.md) as Mesh Generation work, not this spec, because it is tool-specific panel reorg rather than cross-panel chrome. Also: the **Materials panel** is assessed "drop" below, but that verdict was reopened - the panel is kept as a spec-sized item in [`backlog.md`](../backlog.md) and will get its own STUDY, so treat the drop here as superseded._
...
**Help + Diagnostics merge shrinks the panel count.** `panels/help.py` is a 17-row operator cheat-sheet panel (the user called it unusable); `panels/diagnostics.py` is one smoke-test button already behind the `debug_mode` preference poll. The merge: one **Help** panel with an `Open help` button invoking the existing `proscenio.help` popup (move the operator reference into a help topic) plus the smoke-test button when `debug_mode` is on. Net minus one panel, minus one module, and the popup mechanic (`PROSCENIO_OT_help.invoke_popup`) already exists. The merge also clears three code-read findings that live in these exact files: both panels hard-code `help_topic="pipeline_overview"`, which has no `HELP_TOPICS` entry (`diagnostics.py:29`, `help.py:44`), so the `?` opens nothing; `Run Smoke Test` calls raw `self.report` instead of `report_info`, bypassing the log-level gate + prefix (`help_dispatch.py:110`); and the Diagnostics docstring advertises "future addon-health buttons" that do not exist (`diagnostics.py:31-33`). Deleting the Diagnostics module drops the placeholder; the surviving Help button routes through a real topic and the gated report.

**Help-text revision is one editorial sweep, not field polish.** Three sub-jobs share the same pass over `help_topics.py` + the panel tooltip strings: (1) revise the `?` / tooltip copy so each panel's button explains the panel and each subpanel explains its own specifics without leaking, and fix the status-badge legend that still names `TOOL_SETTINGS` (it no longer means "only works in Blender"); (2) add a help block clarifying the Reproject UV / UV bounds / Texture Region purpose (the projection itself is fixed by the deterministic-projection PR); (3) fix three strings that state the opposite of the code - the driver-axis PropertyGroup labels say "local rotation" though the operator forces world space (`object_props.py`, `driver.py`; the duplicated axis enum behind these is unified in the same edit, since it is the single source of the label / order / default), the `show_provenance_overlay` description says the GPU overlay "ships later" though it shipped, and the bind error string says "Skinning panel > Bind mode" though the panel is now Weight Paint. All text-only; the code is already correct.
...
| left-align-names | 2 | 1 | 1 | 1 | done | UIList alignment fix (split row + LEFT-aligned label, star kept right). Landed via the spec 043 PR since that PR already restructured these `draw_item` rows. Do not redo in this spec's polish batch. (2026-06-17) |
| frame-unhide-on-click | 3 | 2 | 2 | 2 | now | Closes the validation loop; one operator, three panels benefit. |
| indented-tree | 2 | 3 | 3 | 2 | drop | Reopened then dropped 2026-06-17 (user call): the full parent-nested tree with expand/collapse can only be _simulated_ in a Python `UIList` (Blender's Tree View is C-only), so keep the standard native list + search and left-align labels only. See [`dropped.md`](../dropped.md) `outliner-hierarchy-tree`. |
...
| tooltip-help-copy-revision | 2 | 2 | 1 | 2 | now | One editorial pass over the `?` / tooltip copy + the stale `TOOL_SETTINGS` status-badge legend; the spec already owns the help system. |
| reproject-uv-purpose-help | 2 | 1 | 1 | 2 | now | One help block on the Reproject UV / UV bounds / Texture Region purpose; rides the deterministic-projection PR that fixes the operator. |
| stale-help-strings | 2 | 1 | 1 | 1 | now | Three in-code strings that contradict the code (driver-axis "local rotation", provenance "ships later", bind error "Skinning panel"); text-only, and unify the duplicated axis enum that is the single source of the driver labels. |
| texture-region-hide-for-mesh | 2 | 1 | 1 | 1 | now | Hide the Texture Region subpanel when the element is a mesh (Polygon2D does not use it); addendum-flagged, pure chrome. |
| select-issue-view-layer-guard | 3 | 2 | 2 | 1 | now | Guards `select_issue_object` against an out-of-view-layer object; same function as frame-unhide, lands in the same edit. |
| help-diagnostics-panel-bugs | 1 | 2 | 1 | 1 | now | Wrong `pipeline_overview` topic + smoke-test report-gate bypass + dead placeholder copy; all obviated or fixed by the Help + Diagnostics merge. |
...
| materials-panel | 2 | 4 | 4 | 4 | reopened | Originally dropped (path repair duplicates native Find Missing Files; inspection/bulk surface speculative). Verdict reopened 2026-06-16: kept as a spec-sized item in [`backlog.md`](../backlog.md) and will get its own STUDY. See the reconciliation note at the top of this file. |
...
**19 now, 1 done, 0 defer, 5 gate, 3 drop, 1 reopened.** (2026-06-17: left-align-names landed via the spec 043 PR; indented-tree dropped - see the rows above. 2026-06-18: six items pulled in from the backlogs - the tooltip / help-copy revision, the Reproject UV purpose help, the three stale help-strings, the Texture Region hide-for-mesh, the Select-Issue-Object view-layer guard, and the three Help / Diagnostics panel bugs - because each shares a file or theme this spec already touches; see the surface notes and the backlog-pointer trail.) Of the three new surfaces: onion-skin-overlay is pruned (ecosystem and native tooling already cover the real pain), joystick-slider-blend stays gated on the first parametric-face character, and materials-panel's drop was reopened 2026-06-16 - it is kept as a spec-sized item in [`backlog.md`](../backlog.md) and will get its own STUDY (see the reconciliation note at the top). The recommendation is six PRs:

1. The blocking help-orphan re-wire (both orphans + the reverse-coverage test).
2. The Reproject UV planar-projection rewrite, plus the purpose-clarity help block for Reproject UV / UV bounds / Texture Region.
3. **One batched polish PR** - header-mesh-name, clamp-initial-frame, rename-initial-frame, the centered-vs-origin help line, Texture Region hide-for-mesh, frame-unhide on issue click (with the out-of-view-layer guard in the same `select_issue_object` edit), the see-also URL migration (closes two scope rows), and the panel-helper consolidation as its final mechanical commit - all verified by a single GUI smoke pass (left-align outliner names already shipped via 043).
4. The Help popup button + Diagnostics merge, which also fixes the wrong `pipeline_overview` topic, routes Run Smoke Test through the report gate, and drops the Diagnostics placeholder copy.
5. The help-text + tooltip revision sweep: the `?` / tooltip copy revision, the stale `TOOL_SETTINGS` status-badge legend, and the three lying help-strings (unifying the duplicated driver-axis enum that is their single source).
6. The guide-doc rename sweep, after the UI renames land so the prose matches the shipped editor.
