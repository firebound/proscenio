...
- **wpaint-named-snapshots** - the weight snapshot UX gives no sense of which save point a restore targets; add named snapshots.
...
- **list-multiselect** wires `invoke` + the event read into the Bone select path (`selection.py:126-157` is single-select only today because `execute` is called directly without an `invoke`). The Outliner already has multi-select; bones gain it; "pick one" lists stay single. The `template_list` single-`active_index` limit means multi-highlight is an approximation (per-item selected state + a custom mark), so the component owns that approximation in one place. Size **M**.
- **proscenio-list-cross-deselect** is the one item with a design caveat: native Blender keeps a per-list `active_index`, so the agents flagged "by design". But the user observed the cross-list highlight as confusing, and the resolution is concrete: make the active-row highlight follow the real Blender active object/bone (the 043 outliner identity-sync), so a row reads as active only when its object/bone is the actual active one. This is highlight-follows-selection, not independent indices. Size **S-M**, lands with the component.
...
- **show-provenance-overlay-toggle**: the panel toggle (`weight_paint.py:338`) registers no draw handler; the provenance overlay handler is registered only inside the Edit Weights modal (`edit_weights.py:97-99`) and torn down on finish. The standalone toggle is dead UI. The correct fix is to remove the standalone toggle and keep the control only inside the modal (where the handler lives), not to build a persistent GPU overlay - `decisions.md` "Weight paint mode sync" locks that the modal is the only context where the overlay's state mutations are legal. Size **S**.
...
- **wpaint-named-snapshots**: the sidecar carries per-vertex `provenance` (`core/skinning/sidecar_schema.py`) and the panel offers one "reset to last saved" with no sense of which point it restores. Add an additive `snapshots: list[NamedSnapshot]` to the sidecar (pre-launch, no version bump), a `restore_named_snapshot(name)` operator, and a snapshot list (a consumer of the component). Size **M**.
