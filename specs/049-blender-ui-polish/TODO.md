...
- [ ] list-multiselect: reuse the Outliner's per-row-marker pattern (the radio dot reading `obj.select_get()`, not `template_list`'s single `active_index`); wire `invoke` + the `event.shift`/`event.ctrl` read into the Bone select path ([`operators/selection.py`](../../apps/blender/operators/selection.py), single-select today) so bones get it. "Pick one" lists stay single; do not replicate multi where it does not apply.
- [ ] proscenio-list-cross-deselect: make the active-row highlight track the real Blender active object/bone for the object/bone lists (Outliner, Skeleton, Slots) via the 043 identity-sync. The Animation actions list stays independent - its active row is the Skeleton-picked armature's active action, not an object.
...
- [ ] wpaint-named-snapshots: build both manual named save points (unlimited) and rolling auto-snapshots (last 3 per paint session). Add an additive `snapshots` list to the weight sidecar ([`core/skinning/sidecar_schema.py`](../../apps/blender/core/skinning/sidecar_schema.py), pre-launch so no version bump) carrying both kinds with a kind flag, a `restore_named_snapshot(name)` operator, and a snapshot list (a component consumer) showing the named saves plus the last-3 auto entries; headless tests for manual save -> paint -> restore-by-name and the rolling-3 cap.
