...
Skeleton STUDY - decisions framed, not yet locked. The feature is additive and rides a settled contract: spec 055 already defines what survives a re-import, so this spec's work is scoping that contract to a single manifest entry and wiring the entry point into the Element panel. The three open decisions are all "resolution" questions - how to map an Element to its entry (decision 1), what to do when that entry shifted or vanished (decision 2), and where the manifest path comes from (decision 3) - none of which reopens an architectural call. The likely shape is name-keyed resolution with explicit warnings now, stable-id as later hardening; the 055 contract honored per-entry; the manifest path remembered from first import. Size estimate pending lock; likely M (factor the per-entry path out of the manifest loop + panel control + resolution + tests), with a clean reuse of `stamp_mesh` / `stamp_sprite` / `_find_existing`.
