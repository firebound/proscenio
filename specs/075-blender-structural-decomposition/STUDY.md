...
| D4 | **Coordinate the cross-spec file overlaps with 074.** `automesh_authoring.py` (075 large split) is also touched by 074's `automesh-snap-math-dup` (Phase 3) and the `automesh-modal-preamble` DRY fold; `photoshop-planes.py` by 074's `place-and-tag-hidden-dep` + `planes-placement-math`. Land the 074 cleanup first (smaller), then the 075 split rebases onto it. **STATUS (PR #182):** all four of those 074 Phase 3 items have LANDED - the placement math is in `core/psd/placement.py`, `_place_and_tag` takes `spacing`, the pen axis-lock is in `core/automesh/stroke_geometry`, and the poll/invoke preamble is in `operators/automesh/_authoring_preconditions.py`. The 075 splits now rebase onto the slimmer files; also keep the two 074 accessors (`is_running()`, `authoring_state()`) on their operator shells. | Avoids two specs fighting the same file; the 074 cleanups are smaller and make the 075 split cleaner. |
