...
| **Manual-test surface owned by the QA Companion tool** | Spec 040 was a one-time automatic pass mapping every user-reachable control + flow (452 items) plus a code-read audit (176 findings). Those outputs are living data (walked, edited, pruned over time), not a finished plan, so they live in `tools/qa-companion/` (`checklist/` + `findings.md`) and the tool round-trips edits straight back to the `.md`. Spec 040 was retired (folder removed; recorded in [`_index.md`](_index.md)). | The block format changes shape (the tool's parser/serializer + the format legend in its README move together), or the tool is replaced. |
