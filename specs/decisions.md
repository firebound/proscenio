...
- **Follow convention = object-parent + a `Child Of` constraint that cancels the bone rest + the `slot_bone` field; never real bone-parenting.** The constraint bakes `inverse_matrix = (armature.matrix_world @ pose_bone.matrix).inverted()` (full location + rotation + scale), the headless Set-Inverse and the Blender twin of the importer's `get_skeleton_rest().affine_inverse()` cancel, so the slot rides only the bone's pose delta. Real bone-parenting would inherit the in-plane bone's rest orientation and tilt the flat attachment quads out of the picture plane - the failure the object-parent convention avoids. No Godot change: the importer already reconstructs this follow.
...
- **`create_slot`'s pose-bone path migrated onto the convention.** It object-parents the new Empty to the armature, anchors it at the bone tail, and binds via the shared helper instead of setting `parent_type = "BONE"`.
- **Bind and unbind normalize a legacy real BONE parent first.** A slot from the old bone-parented shape is flipped to object-parent (world transform preserved) before the follow is added or removed, so bind never double-drives (a Child Of stacked on a live bone parent) and unbind never leaves the slot bone-driven.
- **Re-bind is the cheap inverse recompute (the Set-Inverse caveat).** The inverse is baked at bind time, so moving a bound slot needs a rebind; running bind again removes the single named "Proscenio Slot Follow" constraint and re-adds it, never stacking duplicates.
