...
- **Scope boundary (open follow-on)**: the contour pen edits the OUTER contour of an existing mesh element and ends at APPLY (a one-shot session). A from-blank pen-creation tool (first click drops the first vert, Illustrator-style) and persistent re-editing of an applied mesh are deliberately out of spec 066's scope - logged as `mesh-pen-authoring` in the UI-feedback backlog. Spec 070 closed this follow-on: a standalone Manual Mesh modal now authors a contour from blank and re-opens an applied mesh for editing.
