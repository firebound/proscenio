...
- **The multiGet IPC fast-path is deferred, not shipped blind.** The single-walk speedup (a `batchPlay` multiGet replacing the DOM walk) is held until a live-Photoshop session can validate the descriptor against a real PSD, because shipping an unverifiable host descriptor is the same mock-vs-reality gap that caused the original crash. Rationale: the hardened DOM adapter is correct and fast enough for real (flat) Proscenio documents; the speedup matters most on the large-doc tail, which is itself gated. Revisit: a GUI session that validates the descriptor, then build it as a try -> DOM-fallback fast path. Plan in [`deferred.md`](deferred.md).
