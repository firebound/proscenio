...
Open work is tracked in the per-domain backlog files indexed from [`backlog.md`](backlog.md); this file is the consolidated home for locked decisions. It carries the cross-cutting architectural calls plus the per-feature studies made while building each feature. The hand-written planning docs that once held this rationale were discarded once their features shipped, so every decision worth keeping was mirrored back here - this file, not a spec folder, is now the record.
...
| **Lockstep product versioning** | The three apps (Blender addon, Photoshop plugin, Godot importer) are one coupled pipeline - the addon emits a `format_version` the importer must read - so they ship under a single SemVer (`vX.Y.Z`), bumped by the highest severity across components; unchanged apps re-stamp and ride along. The version is a compatibility coordinate ("same number = compatible"), not a per-app change counter, so per-app detail lives in the CHANGELOG. Replaces the earlier per-component scheme (three independent SemVer streams). `format_version` stays independent of the product version. Detail in [`.ai/conventions/layout.md`](../.ai/conventions/layout.md) "Versioning". | A component gains an independent consumer or release cadence (a user upgrades one without the others) - which is what per-component SemVer is actually for. |
...
| **`apps/` + `packages/` + `scripts/` + `tools/` split** | `apps/` holds distributable bundles (Blender, Photoshop, Godot) plus the docs site. `packages/` holds shared building blocks consumed by apps (pydantic models, codegen CLI, fixtures, validator). `scripts/` is one-off, run-directly helpers with no own dependencies. `tools/` holds standalone dev/QA tools that are their own package (own deps/build/tests), not shipped and not a shared lib (e.g. `tools/qa-companion`). Boundary: own dependency manifest/build means `tools/` or `packages/`, never `scripts/`. Canonical detail in [`.ai/conventions/layout.md`](../.ai/conventions/layout.md). | The second pnpm/TypeScript package (`tools/qa-companion`) stays a standalone pnpm project; a third would warrant a root-level pnpm workspace alongside the uv workspace. |
| **Manual-test surface owned by the QA Companion tool** | Spec 040 was a one-time automatic pass mapping every user-reachable control + flow (452 items) plus a code-read audit (176 findings). The walkable surface is living data (walked, edited, pruned over time), not a finished plan, so the `checklist/` lives in `tools/qa-companion/` and the tool round-trips edits straight back to the `.md`. The code-read audit (`findings.md`) was triaged on 2026-06-15 and retired: its real code issues went to the bug backlogs, its doc/help-text gaps to [`backlog-docs.md`](backlog-docs.md), and the discarded findings were dropped - so `findings.md` was removed (recoverable from git history) and only `checklist/` remains under the tool. Spec 040 was retired (folder removed; recorded in [`_index.md`](_index.md)). | The checklist block format changes shape (the tool's parser/serializer + the format legend in its README move together), or the tool is replaced. |
...
- **Storage split by field intent (revised).** The first cut mirrored every PropertyGroup field to a raw Custom Property and read PropertyGroup-first / CP-fallback. That uniform mirror proved over-broad: editor-only fields need no CP, while animatable / driver-target fields (`frame`, slot index) must be CP-canonical because Blender cannot keyframe a field nested in a PropertyGroup. The split-by-intent contract is locked; implementation is deferred to before 1.0.0.
...
- **Targeted boundary rewrite, not from-scratch.** The unusable plugin was fixed by rewriting only the two host-boundary layers (the IPC adapter and the export writer) behind their existing `AdaptedDocument` / `Manifest` shapes; the tested logic core (planner, tag parser, manifest builder, validator) was left untouched. Rationale: the rot was at the host boundary, not in the pure logic, so a from-scratch rewrite would discard tested code and multiply the manual re-validation cost.
- **Every PS-API collection read is normalized against null and non-iterable.** `doc.layers`, a group's `.layers`, `activeLayers`, and the guides read all route through a guard that returns `[]` for null / undefined / non-iterable. Rationale: the `object null is not iterable` crash that made the Tags panel and export unusable came from a host getter returning `null` where the type said non-null, in a spot the tests had mocked as a clean array; the guard is the standing discipline and the tests now drive the real host failure modes (null collection, a rejecting `duplicate`). Revisit: none, baseline rule for host reads.
- **Tag-write targets resolve by stable layerID, name-path as fallback.** `renameLayer` looks the target up by `findLayerById(node.id)` first and falls back to the name-path. Rationale: the cached name-path goes stale when a parent group is renamed between the tree build and the click, and the id also disambiguates duplicate sibling names; the tag tree is keyed on the id for the same reason (no remount on rename). The click-to-select and legacy-migration paths still resolve by name - that half of the duplicate-name finding is tracked in [backlog-photoshop.md](backlog-photoshop.md).
- **Export is partial, not all-or-nothing.** A per-layer try/catch records a failed PNG write as `{ ok: false }`; the manifest is written for the entries whose PNGs landed and the failures are named, instead of suppressing the whole manifest on one bad layer. Rationale: one problematic layer used to block the entire export; the invariant that the manifest never references a PNG missing from disk is kept by excluding the failed entries.
- **The multiGet IPC fast-path shipped as a try-multiGet -> DOM-walk fallback (spec 048).** The descriptor was validated against a real PSD (`debug/doll_tagged_debug.psd`, id 59) in a live session on 2026-06-18: one `batchPlay` `multiGet` over `{ _ref: "document", _id }` with `extendedReference` keys `["name", "layerID", "visible", "layerSection", "bounds"]` and `count: -1` read the 44-layer stack in ~3 ms versus ~41 ms for the per-property DOM walk, rebuilt node-for-node. The hardened DOM walk stays in the tree as the always-correct safety net: the reader returns `null` on any failure (no doc id, a throw, a wrong-shape result, an empty list, or a non-empty list that folds to an empty tree) and the caller falls back. The live descriptor differs from the type stub in ways only a real PSD could confirm - `layerSection` is an `{ _enum: "layerSectionType", _value }` envelope (not a bare string), each bounds side is a `{ _unit, _value }` envelope, the id key is `layerID`, and layers enumerate bottom-up with each group carrying both a start marker (the name) and an end divider - so the field mapper isolates those as the single swap point. Supersedes the earlier "deferred, not shipped blind" call.
- **The async tree read uses a cancellation-guarded effect, never Suspense (spec 048).** `useTagTree`'s read loop awaits the multiGet behind an `inFlightRef` (no overlapping reads doubling the IPC), a `pendingRef` (a trigger arriving mid-flight does exactly one more read instead of being dropped), and a `mountedRef` (no stale write after unmount), keeping the existing `lastSyncAtRef` poll de-dupe; `useExportPreview` shares the same fast read via `previewExportAsync` with a latest-wins token. Suspense was rejected because the poll refresh would unmount the user's open dropdowns / expanded rows on every tick, defeating `buildTagTreeReusing`'s ref preservation. Revisit: collapsing both read paths to a single shared `adaptDocument` snapshot per tick is the remaining sibling, deferred (small win now that both paths use the cheap read) in [`deferred.md`](deferred.md).
- **The tag-list re-render bug was the shared per-row `busy` prop, not `tagRowEqual` (spec 048).** A single `busy` boolean was a prop of every `TagRow`, so one rename toggled it tree-wide and re-rendered every row twice; `tagRowEqual` was correct and only carried a now-dead `busy` comparison. Fixed by dropping the per-row disable entirely - renames serialize through `core.executeAsModal`, so the disable was defensive UI, not a correctness gate.
- **The doll tag oracle is split from the real character.** `doll_tagged.psd` is the clean end-to-end authoring figure; `debug/doll_tagged_debug.psd` is the kitchen-sink taxonomy oracle the committed Python parity test validates. Rationale: the oracle is intentionally deformed and must never be confused with the authoring flow, so isolating it under `debug/` keeps the two apart.

### Slot bone-follow

- **Two follow shapes are supported; the object-parent + `Child Of` constraint is the facilitated default.** Both a `slot_bone`-bound slot (object-parent + a Child Of constraint whose inverse cancels the bone rest) and a hand-authored real bone parent (`parent_type == "BONE"`) are first-class: each exports the same bone name and Godot rebuilds them identically (it parents the Node2D under the Bone2D and cancels the rest regardless of how Blender authored the follow). The constraint shape is preferred and offered by the Bind to Bone button because it stays flat for any bone orientation and lives in a separate, removable layer - unbinding returns the Empty to its clean authored transform instead of stranding it with the bone rest baked into local. Real bone-parenting is a permanently supported hand-authored shape, never deprecated or stripped.
- **The Child Of inverse is `(armature.matrix_world @ pose_bone.matrix).inverted()` (full location + rotation + scale).** The headless Set-Inverse, the Blender twin of the importer's `get_skeleton_rest().affine_inverse()` cancel, so the slot rides only the bone's pose delta. No Godot change: the importer already reconstructs this follow.
- **A real bone parent only renders correctly for bones pointing into the screen (+/-Y); an in-plane bone collapses the attachment quads.** Verified empirically: a flat quad bone-parented to a +Y bone exports at full area, the same +Z (in-plane) bone-parent exports at zero area (edge-on), while the constraint shape stays full-area for both. The constraint cancels the rest so it never collapses; the panel flags a bone-parented slot whose bone lies in the picture plane (`bone_parent_collapses`, `abs(world_dir.y) < 0.7`) and points the user to Unbind + Bind to Bone.
- **The shared `slot_parent_bone` resolver reads `slot_bone` first, then a real BONE parent - the same order the writer emits.** One definition feeds the Active Slot panel, the slot validators, and (by matching the writer's order) the export, so the three never disagree about the followed bone; a correctly-bound slot stopped reading as `(unparented)` and stopped tripping the false "no parent bone" error.
- **`create_slot`'s pose-bone path defaults to the constraint shape.** It object-parents the new Empty to the armature, anchors it at the bone tail, and binds via the shared helper instead of auto bone-parenting; a user who hand bone-parents an Empty still gets a recognized, exportable slot.
- **Bind refuses when the slot already follows (constraint or bone parent); the two-click flow is Unbind then Bind.** The operator never silently converts or strips a hand-authored shape - it reports "already follows via X; Unbind first". Unbind removes whichever relation is live (the named "Proscenio Slot Follow" constraint or a real bone parent, world transform preserved) and clears `slot_bone`, leaving the Empty object-parented and inert. Moving a bound slot needs a rebind (the Set-Inverse caveat), which is Unbind then Bind.
- **The bind operator resolves its armature object-parent-first, then the Skeleton picker, then the export armature, and picks the bone via a `prop_search` dropdown.** Revisit: a user binds a slot parented to a non-armature and is surprised the follow targets the picker / export rig rather than the slot's own parent - then enforce or surface the "object-parent to the followed armature" convention at bind time.

### Outliner selection

- **The stale-row crash is guarded at the operator boundary, not in shared `select_only`.** `PROSCENIO_OT_select_outliner_object.execute` pre-checks the object is in the view layer; a blanket suppress inside the shared `select_only` helper would make the slot / camera / bone callers silently no-op when an object genuinely cannot be selected.
- **`template_list.active_index` is a source-collection index, so the by-name write was already correct.** Blender maps the source index to the visible row via `flt_neworder`, so the highlight bug was solved by hiding out-of-view-layer rows (`row_visible`), not by rewriting the index sync. The identity-to-index resolver lives in `apps/blender/core/outliner_view.py` (`source_index_for_name`), shared with the slot list.
- **Favorites are filter-only; the second search bar was dropped for Blender's native `filter_name`.** The star survives the favorites-only filter but does not reorder to the top; the `scene.objects` source migration was rejected in favour of the lighter in-view-layer `row_visible` filter, which keeps the rank / sort / index mapping untouched.
...
- **The provenance overlay refreshes at stroke end, not live.** The modal brackets a native brush operator with no per-sample hook, so the ceiling is a stroke-end refresh that tags all `VIEW_3D` areas (a single `context.area.tag_redraw()` missed the release region). Carried honestly into the QA checklist observe text.
- **"Empty vertex group" = no vertex weighted above zero.** The clear-empty operator uses that weight>0 predicate and confirms the names before deleting; empty groups are safe to drop.

### Skeleton chrome and animation target

- **Quick Armature Esc shipped labels-only; destructive cancel stays gated.** `_sweep_empty_armature` already deletes only an auto-created empty rig, so Esc and Enter are identical on the data - the fix was a dynamic header label (Return = "finish", Esc hint tracks `_last_bone_name`). A truly destructive Esc (discard authored bones) is gated on a live GUI repro showing users expect it; see [`gated.md`](gated.md).
- **Animation assign-action targets the Skeleton picker, warns and cancels when empty.** Routes through `resolve_skeleton_target`; the old "first armature in scene" heuristic was the behaviour bug and is removed.
- **Cross-panel convention: a panel acting on another panel's selection draws `draw_target_readout` ("Target: Skeleton \<name\>").** Mesh Generation, Weight Paint, and Animation show it; the owner panel (Skeleton) and child sub-boxes (WP Bind) do not repeat it.

### Slots list UX

- **No reusable list component was built; multi-select shipped as a capability of the operator, not a widget.** Identity-aware index sync lives in `core/outliner_view.py` (`source_index_for_name`); the Outliner select operator reads `event.shift` / `event.ctrl` for extend / toggle. A shared list wrapper is deferred until a genuine third consumer needs it.
- **The attachment list is a custom-draw column over the derived `empty.children`, with a `scale_y` cap, never a synced `CollectionProperty`.** `template_list` needs a real collection and a single `active_index`; the attachments are dynamic children, so the native-scroll / filter / persisted-active-row upgrade is gated on an observed desync or concrete need. The slot list itself (a real collection) did move to `template_list` (`PROSCENIO_UL_slots`).
- **The empty-slot signal is owned solely by the validator error.** The panel no longer draws an inline INFO duplicate; the two messages ("empty slot" and "no mesh children") meant the same thing.

### Godot import verification

- **A Sprite2D region is enabled only when a texture resolves.** A texture-less sprite ships region-disabled (an enabled zero-area region draws nothing), mirroring the mesh builder leaving UVs raw without a texture.
- **Colliding node names are disambiguated by Godot's numeric `add_child` suffix (`_001`), never a kind prefix.** Animation tracks resolve their targets by leaf name via `find_child`, so a prefix would churn the lookups.
- **Bone-attached slots cancel the bone's full cumulative rest, not the parent-local rest.** `Bone2D.get_skeleton_rest()` returns the parent-local rest in this Godot version; cancelling the cumulative rest (`skeleton.global_transform.affine_inverse() * bone.global_transform`, then inverted) anchors the absolute-baked attachments at the skeleton origin where authored.
