...
- **i18n-locale-tables** - SHIPPED 2026-07-04 (spec 072, PR #185): the trigger fired (the maintainer, a native pt-BR speaker, asked for Brazilian Portuguese). Both surfaces gained a real pt-BR locale. Blender: the per-locale tables split into a bpy-free `core/i18n_locales/` package (one module per language) that `core/bpy_helpers/i18n.py` folds + registers; an AST extraction script (`scripts/blender/extract_i18n.py`) + a reverse-coverage test guard the catalog; report messages route through an injectable translator in `core/_shared/report.py`; 643 msgids translate at 100% (static strings + the UILayout `text=` overrides Blender auto-translates via `translate=True`, plus the help bodies), with f-string dynamic messages deferred (`extract_i18n.py --unwrapped`). Docs: the Docusaurus `pt-BR` locale (chrome, Portuguese search stemmer, locale dropdown) + the 31 hand-authored pages translated, the auto-generated schema reference left English-only behind per-page fallback. English stays the canonical msgid on both surfaces; adding a language is append-only. Kept here as the record. Original framing: `core/i18n.py` registered an empty `TRANSLATIONS`, the docs site was English-only, and the copy-churn prerequisite was met by spec 064 (translation-stable help copy).
